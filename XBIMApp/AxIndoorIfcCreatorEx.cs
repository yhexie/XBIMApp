...
namespace XBIMApp
{
    class AxIndoorIfcCreatorEx
    {
                ...
                if (model != null)
                {
                    ...
                    if (m_WallPolylines.Count > 0)
                    {
                        for (int i = 0; i < m_WallPolylines.Count; i++)
                        {
                            ...
                            {
                                IfcWallStandardCase wall = CreateCurvedWall(model, x, thickness, 2400);
                            ...
                            else if (x.polyline.Count == 2)
                            {
                                ...
                                IfcWallStandardCase wall = CreateWall(model, fromX, fromY, toX, toY,thickness, 2400);
                                if (m_WallPolylines[i].m_Doors != null)
                                {
                                    List<int> doors = m_WallPolylines[i].m_Doors;
                                    for (int i0 = 0; i0 < doors.Count; i0++)
        ...
        public IfcWallStandardCase CreateCurvedWall(IfcStore model, AxPolyline2d curve_,  double width, double height)
        {
            //启动事务
